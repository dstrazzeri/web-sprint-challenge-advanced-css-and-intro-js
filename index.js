...
const yrs = "1900 - 2000";
var years_as_list = yrs.split(" - ");
console.log(years_as_list);

function get20s(array, years){
  const newArray = [];
  for(let i = 0; i < array.length; i++){
    if(array[i].years >= yrs){
...
console.log(get20s(artists, yrs));
...
function removeArtist(arr, artistIndex) {
  for (let i = 0; i < arr.length; i++){
    if (arr[i] === artistIndex){
     arr.splice(i, 1)
    }
  }
  return arr.length - 1;
...
console.log(lotsOfArt(artists, 101))
